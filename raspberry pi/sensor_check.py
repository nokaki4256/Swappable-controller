...
def main_window(List, lay_sel):
    ...
    layout = [[eg.Frame("Main Selsct",[[eg.Button("コントロールモードへ移行",key = "control_mode")],
                                       [eg.Button("Exit")]],),
                eg.Frame("操作説明画面",
                         [[eg.Text("LR:"),eg.Text("", key = "LR")],
                          [eg.Text("UD:"),eg.Text("", key = "UD")],
                          [eg.Text("Length:"),eg.Text("", key = "Length")],
                        [eg.HSeparator()],
...
def check():
    ...
    while True:
        ...
        if event in "timeout":
            ...
            try:
                bus.write_block_data(slave_address[0], 7, command)
            except OSError:
                print("通信がおかしい？")
            try:
                recieve_data = bus.read_i2c_block_data(slave_address[0], 0, 21)
                
                Length = 10. * recieve_data[19] + 0.1 * recieve_data[19]
                ...
                LR = recieve_data[1] + 0.01 * recieve_data[2]
                LR = round(LR,2)
                if recieve_data[0] == 255:
                    LR = -LR
                UD = recieve_data[4] + 0.01 * recieve_data[5]
                UD = round(UD,2)
                if recieve_data[3] == 255:
                    UD = -UD
            except OSError:
                print("通信がおかしい その2？")
            except IndexError:
            ...
            wire_calc(LR, UD, Length, 1700.0, 230.0)
                
        if event == "control_mode":
            print("指定したアドレスのデータを取得")
        #sys.settrace(trace)
